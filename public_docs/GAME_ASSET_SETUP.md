...
公开仓库已经包含五套游戏服装的运行资源，克隆后可以直接使用，本页不是正常部署的必需步骤。如果想从自己持有的游戏 PCK 重新生成或替换某套服装，可以使用导入器。导入器只在本机读取你选择的游戏文件，不联网上传，也不会修改游戏安装目录。
...
- **运行资源目录**：仓库里的 `local_game_assets` 已包含五套可用资源。用本工具重新生成的资源默认放在本机应用数据目录，通过 `local_config` 指针覆盖相应的仓库资源。它不是 Live2D/Cubism 模型，也不能改名伪装成 `.model3.json`。
