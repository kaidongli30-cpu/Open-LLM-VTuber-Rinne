...
在**项目目录**找到 `启动凛祢.bat`，右键选择“编辑”，或“打开方式 → 记事本”。
...
把等号后面填成你自己的语音目录。例如，项目在 `D:\AI\Open-LLM-VTuber-Rinne`，解压后有两层同名语音文件夹，就可能是：

```bat
set "TTS_DIR=D:\AI\Open-LLM-VTuber-Rinne\GPT-SoVITS-v2pro-20250604\GPT-SoVITS-v2pro-20250604"
```

上面的地址只是示例，**请填写你实际解压的位置**。只改等号后、引号内的目录，保留 `set` 和双引号，其他代码不用改。
...
2. 双击项目目录里的 `启动凛祢.bat`。
3. 等待语音和后端完成启动；需要审核日记时按窗口提示操作。
4. 双击桌面客户端，与凛祢对话。

TTS 和后端窗口都要保持打开。第一次使用先按[一键启动说明](#start)填写语音目录。
...
2.0.0 或更早的客户端没有逐项选择界面。如果它提示需要选择，先手动下载安装 2.1.0 或之后的客户端，再由新客户端更新原后端；也可以使用下面的终端方法。
