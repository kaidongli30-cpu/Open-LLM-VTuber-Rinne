...
公开仓库**不附带**《凛祢乌托邦》与《凛绪轮回》游戏原文件或转换后的凛祢游戏模型和其他作者私人设置；但**附带作者当前使用的 GPT-SoVITS V2 参考 WAV**，对应的两份 V2 权重通过本项目 Release 下载。你需要使用自己持有的游戏源文件，在**自己的电脑上**运行肖像导入器。作者自制服装与程序代码分开授权；服装为 [CC BY-NC 4.0](assets/rinne-original-outfits/LICENSE.md)，不能未经另行授权用于商业用途。

本指南以 Windows PowerShell 和 CMD 为主。初次安装的目标是“在桌面客户端看到游戏原画凛祢、输入文字并听到 GPT-SoVITS V2 声线回复”。语音翻译继续使用本地 Ollama 的 `qwen3.5:4b-q4_K_M`。公开模板也启用了 SenseVoice 麦克风识别、近期记忆检索、每日子事件与已审核日记的第二层背景；初次安装尚无日记时不会凭空产生背景。不要把私人运行目录或密钥上传到 Git。
...
- **GPT-SoVITS / Ollama**：前者使用凛祢 V2 权重把日语文字合成语音；后者用指定的本地模型把中文回复翻成日语，并用另一个24B本地模型生成每日子事件。它们都要作为独立服务运行，`uv sync` 不会代替安装或启动它们。
- **第二层背景_layer2**：从你审核过的日记提炼出的长期背景,用于让凛祢记住你大致是一个怎样的人，当前的状态怎样……它不同于原始日记；没有日记或尚未审核时不会凭空生成。
